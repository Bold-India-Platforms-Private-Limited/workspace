    ...
    useEffect(() => {
        if (currentWorkspace) {
            ...
            const visibleProjects = user?.role === "ADMIN"
                ...
                : currentWorkspace.projects.filter((project) =>
                    (project.groups || []).some((g) => userGroupIds.has(g.groupId || g.group?.id))
                );

            const visibleTasks = visibleProjects.flatMap((project) =>
                user?.role === "ADMIN"
                    ? project.tasks
                    : project.tasks.filter((t) =>
                        t.groups?.some((tg) => userGroupIds.has(tg.groupId || tg.group?.id))
                    )
            );
            ...
            setStats({
                ...
                completedProjects: visibleProjects
